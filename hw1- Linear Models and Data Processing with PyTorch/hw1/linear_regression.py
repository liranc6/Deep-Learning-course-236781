...
class LinearRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        w_opt = None
        # ====== YOUR CODE: ======
        reg_matrix = self.reg_lambda * len(y) * np.eye(X.shape[1])
        reg_matrix[0][0] = 0
...
class BostonFeaturesTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        X_transformed = None
        # ====== YOUR CODE: ======
        X_inv_features = X[:, [0, 1,13]]  # these fit crim , lstat

        X_inv_features[:, 1] = 1 / X_inv_features[:, 1]
        X_inv_features[:, 2] = 1 / X_inv_features[:, 2]

        X_transformed = self.poly.fit_transform(X)
        X_transformed = np.concatenate((X_inv_features, X_transformed), axis=1)
...
def cv_best_hyperparams(
    ...
    search = sklearn.model_selection.GridSearchCV(model, params,
                                        scoring='r2', #or metrics.make_scorer(mse_score, greater_is_better=False)
                                        cv=kf, refit=True)
    # execute search
    search.fit(X, y)
    best_model = search.best_estimator_
